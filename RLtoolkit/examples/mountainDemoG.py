...
def drawMcarView():
    ...
    displayt.append(
        gDrawLine(mcarTopView, minPosition, 0, maxPosition, 0, 'light gray'))
    ...
    for i in range(int((maxPosition - minPosition) / delta)):
        ...
        if lastx != None:
            displays.append(
                gDrawLine(mcarSideView, lastx, lasty, x, y, 'white'))
        lastx = x
        lasty = y
        pos += delta
    displays.append(gDrawLineR(mcarSideView, goalPosition, \
                               mCarHeight(goalPosition) - 0.09, 0, -.1,
                               'white'))
    drawMcarState(position, velocity, lasta)
...
def drawMcarSide(showpos, showvel, lasta):
    ...
    if lasta == 2:
        oldside.append(
            gDrawArrow(mcarSideView, showpos + d, h, showpos + d + d, h,
                       'white'))
    elif lasta == 0:
        oldside.append(
            gDrawArrow(mcarSideView, showpos - d, h, showpos - d - d, h,
...
def g3dUpdate():
    ...
    for i in range(g3dwindow.res):
        pos = minPosition + (
        (maxPosition - minPosition) * (float(i) / (g3dwindow.res - 1)))
        for j in range(g3dwindow.res):
            vel = (2 * maxVelocity * (
            float(j) / (g3dwindow.res - 1))) - maxVelocity
            loadF(Fs, pos, vel)
            if mCarGoal(pos):
                d = 0.0
            else:
