...
class TileDisplay(graph.Graph):
    def __init__(self, x, y, numtilings=1, memct=8192,
                 title="Tile Display comparison", \
    ...
    def calcTiledata(self, numtilings, memct, floats, ints=[]):
        ...
        for i in range(total):
            for j in range(total):
                ...
                tbdnew = fancytiles.backdiagonaltiles(numtilings, memct,
                                                      newfloats, None, ints)
                ctbdnew = fancytilesc.backdiagonaltiles(numtilings, memctc,
                                                        newfloats, None, ints)
                if tbdnew == tbd and ctbdnew == ctbd:
                    sametbd.append(newfloats)
                elif tbdnew == tbd:
                    psame.append(newfloats)
                    print(("back diagonal tiles same in python but not c", x, y,
                           tbd, tbdnew, ctbd, ctbdnew))
                elif ctbdnew == ctbd:
                    csame.append(newfloats)
                    print(("back diagonal tiles same in c but not python", x, y,
                           tbd, tbdnew, ctbd, ctbdnew))
        ...
        data = [samet, samets, sametd, sametbd, sametdm, sametl, samete, psame,
                csame]
        return data


def showtiles(numtilings, memct, floats, ints=[],
              title="Tile Display comparison", \
...
gAddMenu(GMENU, 'Tile Window', \
         [['1 tiling, memory 1024', lambda: showtiles(1, 1024, [1.0, 2.0],
                                                      title="Tile Display comparison, memory 1024")], \
          ...
          ['1 tiling, memory 2048', lambda: showtiles(1, 2048, [1.0, 2.0],
                                                      title="Tile Display comparison, memory 2048")], \
          ...
          ['1 tiling, memory 4096', lambda: showtiles(1, 4096, [1.0, 2.0],
                                                      title="Tile Display comparison, memory 4096")], \
          ...
          ['1 tiling, safe collision table',
           lambda: showtiles(1, cts, [1.0, 2.0],
                             title="Tile Display comparison, safe collision table")], \
          ['2 tilings, safe collision table',
           lambda: showtiles(2, cts, [1.0, 2.0],
                             title="Tile Display comparison, safe collision table")], \
          '---', \
          ['1 tiling, super safe collision table',
           lambda: showtiles(1, ctss, [1.0, 2.0], start=-2.0, end=7.0,
                             title="Tile Display comparison, super safe collision table")], \
...
showtiles(1, 4096, [2.0, 2.0], title="Tile Display comparison, memory 4096",
          start=-2.0, end=7.0)
