...
def g3DSurface(context, data, numx, numy, color):
    ...
    for m in range(num - 1):
        pieces.append(
            drawPart(context, data, num - 1, m, num - 1, 1 + m, color))
        pieces.append(
            drawPart(context, data, num - 1 - m, 0, num - 2 - m, 0, color))
        for i in range(int(round(.5 * (1 + m)))):
            pieces.append(
                drawPart(context, data, num - 2 - i, m + 1 - i, num - 2 - i,
                         m - i, color))
            pieces.append(
                drawPart(context, data, num - 1 - i, m + 1 - i, num - 2 - i,
                         m + 1 - i, color))
            pieces.append(
                drawPart(context, data, num - 2 - m + i, i, num - 2 - m + i,
                         i + 1, color))
            pieces.append(
                drawPart(context, data, num - 1 - m + i, i + 1, num - 2 - m + i,
                         i + 1, color))
            if ((1 + m) % 2) != 0:
                pieces.append(
                    drawPart(context, data, num - 2 - m + i, i, num - 2 - m + i,
                             i + 1, color))
                pieces.append(drawPart(context, data, num - 2 - m + i, i + 1,
                                       num - 1 - m + i, i + 1, color))
    for mu in range(num - 2):
        m = num - 1 - mu
        for i in range(int(round(.5 * m))):
            pieces.append(
                drawPart(context, data, m - i - 1, num - 1 - i, m - i - 1,
                         num - 2 - i, color))
            pieces.append(drawPart(context, data, m - i, num - 1 - i, m - i - 1,
                                   num - 1 - i, color))
            pieces.append(
                drawPart(context, data, i, num - m + i, i, num - m + i + 1,
                         color))
            pieces.append(drawPart(context, data, i, num - m + i + 1, i + 1,
                                   num - m + i + 1, color))
            if (m % 2) != 0:
                pieces.append(
                    drawPart(context, data, i, num - m + i, i, num - m + i + 1,
...
def graphSurface(context, data, view=(0.55, 0.4, .7), color='white',
    ...
    set3DProjection(vxgs * numx, vygs * numy, vzgs * wzmaxgs, \
                    .5 * numx, .5 * numy, .5 * (wzmings + wzmaxgs), numx)
    window2d = project3DWindow(wxmings, wxmaxgs, wymings, wymaxgs, wzmings,
                               wzmaxgs)
    gSetCoordinateSystem(context, window2d[0], window2d[1], window2d[2],
                         window2d[3])
    pieces.append(g3DSurface(context, data, numx, numy, color))
    color = 'yellow'
    pieces.append(
        gDraw3DLine(context, wxmaxgs, wymaxgs, wzmaxgs, wxmings, wymaxgs,
                    wzmaxgs, color))
    pieces.append(
        gDraw3DLine(context, wxmaxgs, wymaxgs, wzmaxgs, wxmaxgs, wymaxgs,
                    wzmings, color))
    pieces.append(
        gDraw3DLine(context, wxmings, wymings, wzmings, wxmaxgs, wymings,
                    wzmings, color))
    pieces.append(
        gDraw3DLine(context, wxmings, wymings, wzmings, wxmings, wymaxgs,
                    wzmings, color))
    pieces.append(
        gDraw3DLine(context, wxmings, wymings, wzmings, wxmings, wymings,
                    wzmaxgs, color))
    pieces.append(
        gDraw3DLine(context, wxmings, wymaxgs, wzmaxgs, wxmings, wymings,
                    wzmaxgs, color))
    pieces.append(
        gDraw3DLine(context, wxmings, wymaxgs, wzmings, wxmaxgs, wymaxgs,
                    wzmings, color))
    pieces.append(
        gDraw3DLine(context, wxmaxgs, wymings, wzmings, wxmaxgs, wymaxgs,
                    wzmings, color))
    hiddenLinesWanted = True
    pieces.append(
        gDraw3DLine(context, wxmings, wymaxgs, wzmaxgs, wxmings, wymaxgs,
                    wzmings, color))
    pieces.append(
        gDraw3DLine(context, wxmaxgs, wymaxgs, wzmaxgs, wxmaxgs, wymings,
                    wzmaxgs, color))
    pieces.append(
        gDraw3DLine(context, wxmaxgs, wymings, wzmaxgs, wxmaxgs, wymings,
                    wzmings, color))
    pieces.append(
        gDraw3DLine(context, wxmaxgs, wymings, wzmaxgs, wxmings, wymings,
                    wzmaxgs, color))
    hiddenLinesWanted = False
    return pieces
