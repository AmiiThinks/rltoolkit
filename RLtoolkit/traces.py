...
class SimpleTraceHolder:
    ...
    def accumulateTraces(self, flist):
        ...
        if isinstance(flist, (tuple, list)):
            for i in flist:
                self.addToTrace(i, 1.0)
                # accumulating traces for current action
        else:
...
class TraceHolder(SimpleTraceHolder):
    ...
    def __init__(self, mem=8192, minT=0.01, maxN=1000):
        ...
        self.E = [0.0 for item in
                  range(self.n)]  # vectory of eligibility traces
        self.numNonZeroTraces = 0  # current number of non zero traces
        self.nonZeroTraces = [0 for item in range(
            self.maxNonZeroTraces)]  # vector of non zero traces
        self.nonZeroTracesInverse = [0 for item in range(
            self.n)]  # vector back to original position
    ...
    def decayTraces(self, decayRate):
        ...
        for loc in range(self.numNonZeroTraces - 1, -1,
                         -1):  # go through trace list backwards
